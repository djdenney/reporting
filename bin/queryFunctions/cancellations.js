...
async function allCancelRates(days, rates = []) {
    const response = await knex.raw(
        `select round((CANCEL_ORDER.CANCEL_ORDER / TOTAL_ORDER.TOTAL_ORDER) * 100, 0)  as CANCEL_RATE, 
        CANCEL_ORDER.PRICE as $_CANCEL_ORDER_LINE_TOTAL, 
        TOTAL_ORDER.PRICE as $_ORDER_LINE_TOTAL
        from
        (select SUM(CANCELLED_ORDER_LINE_SUB_TOTAL) as PRICE, 
        COUNT(ool.PK) as CANCEL_ORDER
        from default_order.ORD_ORDER_LINE ool
        inner join default_order.ORD_ORDER oo on ool.ORDER_PK = oo.PK
        where 1=1
        AND oo.IS_CONFIRMED = 1
        AND ool.IS_CANCELLED = 1 
        AND oo.MAX_RETURN_STATUS_ID is null
        AND ool.MAX_FULFILLMENT_STATUS_ID = '9000'
        AND ool.UPDATED_TIMESTAMP >= now() - INTERVAL ${days} DAY) as CANCEL_ORDER,
        (select SUM(ORDER_LINE_SUB_TOTAL) as PRICE, 
        COUNT(ool.PK) as TOTAL_ORDER
        from default_order.ORD_ORDER_LINE ool
        inner join default_order.ORD_ORDER oo on ool.ORDER_PK = oo.PK
        where 1=1
        AND oo.IS_CONFIRMED = 1
        AND oo.MAX_RETURN_STATUS_ID is null
        AND ool.CREATED_TIMESTAMP >= now() - INTERVAL ${days} DAY) as TOTAL_ORDER`
    )
